...
public class OSCARConnectWizard extends Wizard implements IConnectWizard, INewWizard {
	...
	public OSCARConnectWizard() {
		super();
	}

	public OSCARConnectWizard(String uin) {
		this();
		this.uin = uin;
	}

	public void addPages() {
		page = new OSCARConnectWizardPage(uin);
		addPage(page);
	}
	...
	public void init(IWorkbench workbench, IContainer container) {
		...
		setWindowTitle(Messages.OSCAR_CONNECT_WIZARD_PAGE_WIZARD_TITLE);
	}
	...
	private void openView() {
		try {
			final MultiRosterView view = (MultiRosterView) workbench.getActiveWorkbenchWindow().getActivePage()
	...
	private void displayMessage(IChatMessageEvent e) {
		...
		Display.getDefault().asyncExec(new Runnable() {
			public void run() {
				MessagesView view = (MessagesView) workbench.getActiveWorkbenchWindow().getActivePage().findView(
	...
	private void displayTypingNotification(final ITypingMessageEvent e) {
		Display.getDefault().asyncExec(new Runnable() {
			public void run() {
				final MessagesView view = (MessagesView) workbench.getActiveWorkbenchWindow().getActivePage().findView(
					MessagesView.VIEW_ID);
				if (view != null)
	...
	public boolean performFinish() {
		...
		final String connectID = page.getUin();
		final String password = page.getPassword();

		// Save combo text even if we don't successfully login
		page.saveComboText();

		connectContext = ConnectContextFactory.createUsernamePasswordConnectContext(uin, password);

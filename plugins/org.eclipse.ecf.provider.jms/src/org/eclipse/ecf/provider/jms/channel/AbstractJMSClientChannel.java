...
public abstract class AbstractJMSClientChannel extends AbstractJMSChannel
	...
	public synchronized Object connect(ID target, Object data, int timeout)
		...
		Trace.entering(Activator.PLUGIN_ID, JmsDebugOptions.METHODS_ENTERING,
				this.getClass(), "connect", new Object[] { targetID, data,
						new Integer(timeout) });
		if (isConnected())
		...
		if (!(target instanceof JMSID))
			throw new ECFException("remote " + target.getName() + " not JMS ID");
		this.targetID = (JMSID) target;

		if (!(data instanceof Serializable)) {
			throw new ECFException(new NotSerializableException(
					"data are not serializable"));
		}
		Serializable result = null;
		try {
			Serializable connectData = setupJMS(targetID, data);
			Trace.trace(Activator.PLUGIN_ID, "connecting to " + targetID + ","
					+ data + "," + timeout + ")");
			result = getConnectResult(targetID, connectData);
			if (result != null && (!(result instanceof ConnectResponseMessage))) {
				throw new ConnectException("Invalid response");
			}
			if (result == null)
				throw new ConnectException("server refused connection");
		} catch (Exception e) {
			throw new ECFException("connect failed to " + targetID.getName(), e);
		}
		ConnectResponseMessage cr = (ConnectResponseMessage) result;
		Object resultData = cr.getData();
		Trace.exiting(Activator.PLUGIN_ID, JmsDebugOptions.METHODS_ENTERING,
				this.getClass(), "connect", resultData);
		return resultData;
	}
	...
	protected void respondToRequest(ObjectMessage omsg, ECFMessage o) {
		...
		try {
			...
			Trace.trace(Activator.PLUGIN_ID, "respondToRequest.sending="
					+ first);

...
public class CollabClient {
	...
	public void createAndConnectClient(final String containerType, String uri, String nickname, final Object connectData, final IResource resource) throws Exception {
		...
		props.put("topic", "admin"); //$NON-NLS-1$ //$NON-NLS-2$
		Event event = new Event("topic", props); //$NON-NLS-1$

		IContainerFactory containerFactory = ClientPlugin.getDefault().getContainerManager().getContainerFactory();
		// First create the new container instance
		final IContainer newClient = containerFactory.createContainer(containerType);
		...
		// this
		ISharedObjectContainer soContainer = (ISharedObjectContainer) newClient.getAdapter(ISharedObjectContainer.class);

		// add to soCOntainer with topic as name
		soContainer.getSharedObjectManager().addSharedObject(newClient.getID(), (ISharedObject) ClientPlugin.getDefault().getEventAdminImpl(), props);
		SharedObjectContainerUI socui = new SharedObjectContainerUI(this, soContainer);
		socui.setup(soContainer, newClientEntry, resource, username);
		// Now connect
		try {

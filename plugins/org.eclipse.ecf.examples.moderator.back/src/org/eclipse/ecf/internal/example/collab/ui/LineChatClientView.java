...
public class LineChatClientView implements FileSenderUI {
	...
	protected void handleTextInput(String text) {
		...
		ID sender = this.lch.getContainerID();

		// parse 'à la' IRC command i.e: '/command=evict&member=memberID
		if (props != null) {
			props.put("ID", sender);
			org.osgi.service.event.Event adminEvent = new org.osgi.service.event.Event("admin", props);
			ClientPlugin.getDefault().getEventAdminImpl().postEvent(adminEvent);
		}

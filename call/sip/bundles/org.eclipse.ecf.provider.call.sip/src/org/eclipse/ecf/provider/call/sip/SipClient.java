...
public class SipClient implements SipListener {
	...
	public void processRequest(RequestEvent requestReceivedEvent) {
		...
		System.out.println("\n\nRequest " + request.getMethod()
				+ " received at " + call.getSipStack().getStackName()
		...
		if (request.getMethod().equals(Request.BYE)) {
			...
			try {
				serverTransactionId.sendResponse(SipCall.getMessageFactory()
			...
			if (response.getStatusCode() == Response.OK) {
				if (cseq.getMethod().equals(Request.INVITE)) {
					System.out.println("Dialog after 200 OK  "
							+ call.getResponseDialog());
					System.out.println("Dialog State after 200 OK  "
							+ call.getResponseDialog().getState());
					ackRequest = call.getResponseDialog().createAck(
							((CSeqHeader) response.getHeader(CSeqHeader.NAME))
									.getSeqNumber());
					System.out.println("Sending ACK");
					call.getResponseDialog().sendAck(ackRequest);

					String okSdpData = new String((byte[]) response
							.getContent());
...
//					new SessionDescriptionImpl().resolveOkSDP(okSdpData);
					...
					call.getResponseDialog().sendRequest(
							call.getSipProvider().getNewClientTransaction(
									call.getResponseDialog().createRequest(
											"REFER")));
					...
					if (call.getResponseDialog().getState() == DialogState.CONFIRMED) {
						...
						Request byeRequest = call.getResponseDialog()
								.createRequest(Request.BYE);
						ClientTransaction ct = call.getSipProvider()
					...
					System.out
							.println("User Registration Successful. OK received for REGISTER.");
				} else if (cseq.getMethod().equals(Request.BYE)) {
					// Do nothing=> incoming call terminated from this side.

					System.out
							.println("Ongoing call for Incoming invite request, terminated by INVITE receiver.");
				}
			} else if (response.getStatusCode() == Response.UNAUTHORIZED) {
				// Used by REGISTER
				System.out.println("Unauthorized recived "
				...
				// Used by INVITE
				System.out.println("Proxy Authentication Required recived "
						+ response.getStatusCode());
				URI uriReq = tid.getRequest().getRequestURI();

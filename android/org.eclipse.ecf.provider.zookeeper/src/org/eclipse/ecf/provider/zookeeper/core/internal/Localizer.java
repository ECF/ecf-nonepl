...
public class Localizer {
	...
	public void init() {
		Executors.newSingleThreadExecutor().execute(new Runnable() {
			public void run() {
				...
				final DiscoveryContainer discovery = DiscoveryContainer
						.getSingleton();
	...
	public void localize(Notification notification) {
		if (notification.getType() == Notification.AVAILABLE) {
			// inform all service listeners about this new service
			for (IServiceListener l : DiscoveryContainer.getSingleton()
					.getAllServiceListener()) {
				l.serviceDiscovered(notification);
			}

			// inform interested service type listeners about this new service
			for (IServiceListener l : DiscoveryContainer.getSingleton()
					.getListeners(
							notification.getAdvertisedService().getServiceID()
									.getServiceTypeID())) {
				l.serviceDiscovered(notification);
			}
			...
			// inform interested service type listeners about this lost service
			for (IServiceListener l : DiscoveryContainer.getSingleton()
					.getListeners(
							notification.getAdvertisedService().getServiceID()
									.getServiceTypeID())) {

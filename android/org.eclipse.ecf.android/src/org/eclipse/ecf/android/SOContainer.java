...
public abstract class SOContainer extends AbstractContainer implements ISharedObjectContainer {
	 ...
	 * so that a LoadingSharedObject can be written to a Parcel
	 * @author Pierre
	 *
	 */
	...
	public static final String NAMESPACE_IDENTIFIER = org.eclipse.ecf.android.StringID.class.getName();
	...
	public SOContainer(ISharedObjectContainerConfig config) {
		Assert.assertNotNull( Messages.SOContainer_Exception_Config_Not_Null, config);
		this.config = config;
	...
	public Namespace getConnectNamespace() {
		...
		return IDFactory.getDefault().getNamespaceByName(getNamespaceIdentifier());
	}

	private String getNamespaceIdentifier() {
...
// TODO [pierre]		Trace.trace(ProviderPlugin.PLUGIN_ID, ECFProviderDebugOptions.CONTAINER, msg + ":" + config.getID()); //$NON-NLS-1$
	}

	protected void traceStack(String msg, Throwable e) {
		// TODO [pierre]		Trace.catching(ProviderPlugin.PLUGIN_ID, ECFProviderDebugOptions.EXCEPTIONS_CATCHING, SOContainer.class, config.getID() + ":" + msg, e); //$NON-NLS-1$
	}
	...
	public static ContainerMessage deserializeContainerMessage(byte[] bytes) throws IOException {
		...
		try {
			...
			Log.i("SOContainer ", Messages.SOContainer_EXCEPTION_CLASS_NOT_FOUND, e);
			printToSystemError("deserializeContainerMessage class not found", e); //$NON-NLS-1$
			return null;
		} catch (final InvalidClassException e) {
// TODO			ProviderPlugin.getDefault().log(new Status(IStatus.ERROR, ProviderPlugin.PLUGIN_ID, Messages.SOContainer_EXCEPTION_INVALID_CLASS, e));
...
//	TODO	ProviderPlugin.getDefault().log(new Status(IStatus.ERROR, ProviderPlugin.PLUGIN_ID, Messages.SOContainer_EXCEPTION_NOT_CONTAINER_MESSAGE, null));
...
// TODO					Trace.catching(ProviderPlugin.PLUGIN_ID, ECFProviderDebugOptions.EXCEPTIONS_CATCHING, this.getClass(), "handleSharedObjectMessage", e); //$NON-NLS-1$
	...
	protected void handleUnidentifiedMessage(ContainerMessage mess) throws IOException {
		// do nothing
//		TODO ProviderPlugin.getDefault().log(new Status(IStatus.ERROR, ProviderPlugin.PLUGIN_ID, IStatus.ERROR, NLS.bind("unidentified message {0}", mess), null)); //$NON-NLS-1$

...
public class RegistrySharedObject extends BaseSharedObject implements
	...
	protected final Map remoteRegistrys = Collections
	...
	public RegistrySharedObject(IContainer container) {
	 ...
	 * 
	 * @see
	 * org.eclipse.ecf.core.sharedobject.BaseSharedObject#dispose(org.eclipse
	 ...
	 * (non-Javadoc)
	 * 
	 * @seeorg.eclipse.ecf.remoteservice.IRemoteServiceContainerAdapter#
	...
	private void addReferencesFromRemoteRegistrys(ID[] idFilter, String clazz,
			IRemoteFilter remoteFilter, List referencesFound) {
	...
	public IRemoteServiceReference[] getAllRemoteServiceReferences(
			String clazz, RegularExpression filter) throws InvalidSyntaxException {
		...
		final IRemoteFilter remoteFilter = (filter == null) ? null
				: new RemoteFilterImpl(filter);
	...
	public IRemoteServiceReference[] getRemoteServiceReferences(ID[] idFilter,
		...
		final List references = new ArrayList();
		// Lookup from remote registrys...add to given references List
		addReferencesFromRemoteRegistrys(idFilter, clazz, remoteFilter,
				references);
		...
		if (references.size() == 0) {
				...
				for (int i = 0; i < idFilter.length; i++) {
					...
					if (target != null) {
						AddRegistrationRequest request = new AddRegistrationRequest(
								target, clazz, filter, first);
						if (i == 0)
							first = request;
						// Add to list of all know
						ourAddRegistrationRequests.add(request);
						addRegistrationRequests.put(request.getId(), request);
						sendAddRegistrationRequest(target, request,
								getAddRegistrationRequestCredentials(request));
					}
				}
			}
			...
			for (Iterator i = ourAddRegistrationRequests.iterator(); i
					.hasNext();) {
				AddRegistrationRequest request = (AddRegistrationRequest) i
						.next();
		...
		// Only look in local remote service registry if idFilter is null OR the
		// idFilter
		// contains the local container ID
		if (idFilter == null
				|| Arrays.asList(idFilter).contains(localContainerID)) {
			synchronized (localRegistry) {
				// Add any from local registry
				addReferencesFromRegistry(clazz, remoteFilter, localRegistry,
						references);
	...
	protected Serializable getAddRegistrationRequestCredentials(
	...
	public IRemoteServiceRegistration registerRemoteService(String[] clazzes,
		...
		if (service == null) {
			throw new NullPointerException(
					Messages.RegistrySharedObject_EXCEPTION_SERVICE_CANNOT_BE_NULL);
		}
		final int size = clazzes.length;

		if (size == 0) {
			throw new IllegalArgumentException(
					Messages.RegistrySharedObject_EXCEPTION_SERVICE_CLASSES_LIST_EMPTY);
		}
		...
		if (invalidService != null) {
			throw new IllegalArgumentException(Messages.RegistrySharedObject_7
					+ invalidService);
		}
	...
	public boolean ungetRemoteService(IRemoteServiceReference ref) {
		...
		if (serviceID == null)
			return false;
		synchronized (localRegistry) {
			RemoteServiceRegistrationImpl registry = localRegistry
					.findRegistrationForRemoteServiceId(serviceID);
			if (registry != null)
				return true;
		}
		synchronized (remoteRegistrys) {
			final RemoteServiceRegistryImpl registry = (RemoteServiceRegistryImpl) remoteRegistrys
					.get(serviceID.getContainerID());
			if (registry != null) {
				return true;
			}
		}
			...
			if (registry != null) {
				...
				if (registrations != null) {
					for (int i = 0; i < registrations.length; i++) {
						...
						unregisterServiceRegistrationsForContainer(registrations[i]
								.getContainerID());
					}
				}
			}
	...
	protected void handleContainerConnectedEvent(IContainerConnectedEvent event) {
		...
		if (getContext().isGroupManager()
				|| event.getTargetID().equals(getConnectedID())) {
			targetID = null;
		}
	...
	protected Object callSynch(RemoteServiceRegistrationImpl registration,
		...
		try {
			...
			while ((timeout - System.currentTimeMillis()) > 0 && !doneWaiting) {
				synchronized (request) {
					if (request.isDone()) {
						...
						// Messages.RegistrySharedObject_15 + requestId);
						doneWaiting = true;
						response = request.getResponse();
						if (response == null)
							throw new ECFException(
									Messages.RegistrySharedObject_EXCEPTION_INVALID_RESPONSE);
					} else {
			...
			if (!doneWaiting)
				throw new ECFException(NLS.bind(
						Messages.RegistrySharedObject_19, Long.toString(call
								.getTimeout())), new TimeoutException(call
						.getTimeout()));
		} catch (final IOException e) {
			log(CALL_REQUEST_ERROR_CODE, CALL_REQUEST_ERROR_MESSAGE, e);
			throw new ECFException(
					Messages.RegistrySharedObject_EXCEPTION_SENDING_REQUEST, e);
		} catch (final InterruptedException e) {
			log(CALL_REQUEST_TIMEOUT_ERROR_CODE,
					CALL_REQUEST_TIMEOUT_ERROR_MESSAGE, e);
			throw new ECFException(
					Messages.RegistrySharedObject_EXCEPTION_WAIT_INTERRUPTED, e);
		}
		// Success...now get values and return
		Object result = null;
		if (response.hadException())
			throw new ECFException(
					Messages.RegistrySharedObject_EXCEPTION_IN_REMOTE_CALL,
			...
			listener.handleEvent(new IRemoteCallStartEvent() {
				...
				public String toString() {
					final StringBuffer buf = new StringBuffer(
							"IRemoteCallStartEvent["); //$NON-NLS-1$
					buf
							.append(";reference=").append(reference).append(";call=").append(call).append("]"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
					return buf.toString();
				}
			});
		}
	...
	protected void fireCallCompleteEvent(IRemoteCallListener listener,
		...
		if (listener != null) {
			listener.handleEvent(new IRemoteCallCompleteEvent() {
				...
				public String toString() {
					final StringBuffer buf = new StringBuffer(
							"IRemoteCallCompleteEvent["); //$NON-NLS-1$
					buf
							.append(";response=").append(response).append(";hadException=").append(hadException).append(";exception=").append(exception).append("]"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
					return buf.toString();
				}
			});
		}
	...
	private static final int ADD_REGISTRATION_REQUEST_TIMEOUT = new Integer(
			System.getProperty("ecf.addregistrationrequest.timeout", "5000")).intValue(); //$NON-NLS-1$ //$NON-NLS-2$
	...
	protected void sendRegistryUpdateRequest() {
		...
		try {
			sendSharedObjectMsgTo(null, SharedObjectMsg.createMsg(
					REGISTRY_UPDATE_REQUEST, getLocalContainerID()));
		} catch (final IOException e) {
	...
	protected void handleRegistryUpdateRequest(ID remoteContainerID) {
		...
		// IRemoteServiceProviderDebugOptions.METHODS_ENTERING, this.getClass(),
		...
		// IRemoteServiceProviderDebugOptions.METHODS_EXITING, this.getClass(),
		// REGISTRY_UPDATE_REQUEST);
	}

	protected AddRegistrationRequest sendAddRegistrationRequest(ID receiver,
			AddRegistrationRequest request, Serializable credentials) {
		...
		try {
			sendSharedObjectMsgTo(receiver, SharedObjectMsg.createMsg(null,
					REQUEST_SERVICE, new Object[] { getLocalContainerID(),
							request, request.getId(), credentials }));
		} catch (final IOException e) {
	...
	protected void handleRequestService(ID remoteContainerID,
			AddRegistrationRequest request, Integer requestId,
		...
		try {
			rf = (request.getFilter() == null) ? null : new RemoteFilterImpl(
					request.getFilter());
		...
		try {
			checkRequestServiceAuthorization(remoteContainerID, request,
					credentials);
		} catch (AccessControlException e) {
			// Log and return...i.e. do nothing
			log(
					"handleRequestService. checkRequestServiceAuthorization exception", e); //$NON-NLS-1$
			sendAddRegistrationRequestRefused(remoteContainerID, requestId, e);
			return;
		}
		synchronized (localRegistry) {
			RemoteServiceReferenceImpl[] srs = (RemoteServiceReferenceImpl[]) localRegistry
					.lookupServiceReferences(request.getService(), rf);
			if (srs != null && srs.length > 0) {
				...
				sendAddRegistrationRequestRefused(remoteContainerID, requestId,
						null);
		}
	...
	protected void sendAddRegistration(ID receiver, Integer requestId,
			RemoteServiceRegistrationImpl reg) {
		...
		try {
			sendSharedObjectMsgTo(receiver, SharedObjectMsg.createMsg(null,
					ADD_REGISTRATION, getLocalContainerID(), requestId, reg));
		} catch (final IOException e) {
	...
	protected void sendAddRegistrationRequestRefused(ID receiver,
			Integer requestId, Exception except) {
		...
		try {
			sendSharedObjectMsgTo(receiver, SharedObjectMsg.createMsg(null,
					ADD_REGISTRATION_REFUSED, getLocalContainerID(), requestId,
					except));
		} catch (final IOException e) {
			log(ADD_REGISTRATION_REFUSED_ERROR_CODE,
					ADD_REGISTRATION_REFUSED_ERROR_MESSAGE, e);
		}
	...
	protected void handleAddRegistrationRequestRefused(ID remoteContainerID,
			Integer requestId, AccessControlException e) {
		if (remoteContainerID == null || requestId == null)
			return;
	...
	protected void handleAddRegistration(ID remoteContainerID,
			Integer requestId, final RemoteServiceRegistrationImpl registration) {
		// Trace.entering(Activator.PLUGIN_ID,
		// IRemoteServiceProviderDebugOptions.METHODS_ENTERING, this.getClass(),
		// ADD_REGISTRATION, new Object[] {remoteContainerID, registration});
		if (remoteContainerID == null
				|| getLocalContainerID().equals(remoteContainerID)) {
			return;
		}
	...
	protected void notifyAddRegistrationResponse(ID remoteContainerID,
			Integer requestId, AccessControlException exception) {
		if (remoteContainerID == null)
			return;
		List pendingRequests = new ArrayList();
		// If the requestId is null, this isn't in response to any particular
		// request,
		// So we get all AddRegistrationRequests
		synchronized (addRegistrationRequests) {
			if (requestId == null) {
				for (Iterator i = addRegistrationRequests.keySet().iterator(); i
						.hasNext();) {
	 ...
	 * @since 3.2
	 */
	protected void localRegisterService(
			RemoteServiceRegistrationImpl registration) {
		final Object localServiceRegistrationValue = registration
				.getProperty(Constants.AUTOREGISTER_REMOTE_PROXY);
		if (localServiceRegistrationValue != null) {
			final RemoteServiceImpl remoteServiceImpl = new RemoteServiceImpl(
					this, registration);
			Object service;
			try {
			...
			properties.put(Constants.SERVICE_CONTAINER_ID, remoteContainerID
					.getName());
			TCPServerSOContainer container = (TCPServerSOContainer) getSOContext()
					.getContainer();
	...
	protected void addLocalServiceRegistration(ID remoteContainerID,
			RemoteServiceRegistrationImpl registration) {
		List containerRegistrations = (List) localServiceRegistrations
				.get(remoteContainerID);
		if (containerRegistrations == null) {
			containerRegistrations = new ArrayList();
			localServiceRegistrations.put(remoteContainerID,
	...
	protected void handleCallResponse(Response response) {
		// Trace.entering(Activator.PLUGIN_ID,
		// IRemoteServiceProviderDebugOptions.METHODS_ENTERING, this.getClass(),
		// CALL_RESPONSE, new Object[] {response});
		final Request request = getRequest(response.getRequestId());
		if (request == null) {
			log(REQUEST_NOT_FOUND_ERROR_CODE, REQUEST_NOT_FOUND_ERROR_MESSAGE,
		...
		if (listener != null) {
			fireCallCompleteEvent(listener, request.getRequestId(), response
					.getResponse(), response.hadException(), response
					.getException());
			return;
		}
		synchronized (request) {
	...
	protected Request sendFireRequest(
		...
		try {
			...
			throw new ECFException(
					Messages.RegistrySharedObject_EXCEPTION_SENDING_REMOTE_REQUEST,
					e);
		}
	...
	protected void handleFireRequest(Request request) {
		// Trace.entering(Activator.PLUGIN_ID,
		// IRemoteServiceProviderDebugOptions.METHODS_ENTERING, this.getClass(),
		// FIRE_REQUEST, new Object[] {request});
		final RemoteServiceRegistrationImpl localRegistration = getLocalRegistrationForRequest(request);
		// Else we've got a local service and we invoke it
		final RemoteCallImpl call = request.getCall();
		try {
	...
	protected void unregisterServiceRegistrationsForContainer(ID containerID) {
		if (containerID == null)
			return;
		final List containerRegistrations = (List) localServiceRegistrations
				.remove(containerID);
		if (containerRegistrations != null) {
			for (final Iterator i = containerRegistrations.iterator(); i
					.hasNext();) {
				final RemoteServiceRegistrationImpl serviceRegistration = (RemoteServiceRegistrationImpl) i
						.next();
				try {
	...
	protected void unregisterAllServiceRegistrations() {
		synchronized (remoteRegistrys) {
			for (final Iterator i = localServiceRegistrations.keySet()
					.iterator(); i.hasNext();) {
		...
		synchronized (remoteRegistrys) {
			...
			if (serviceRegistry != null) {
				...
				if (registration != null) {
					...
					unregisterServiceRegistrationsForContainer(registration
							.getContainerID());
				}
			}
		}
	...
	protected IRemoteServiceUnregisteredEvent createUnregisteredEvent(
		...
		return new IRemoteServiceUnregisteredEvent() {
			...
			public String toString() {
				final StringBuffer buf = new StringBuffer(
						"RemoteServiceUnregisteredEvent["); //$NON-NLS-1$
				buf.append("localContainerID=").append(getLocalContainerID()); //$NON-NLS-1$
				buf
						.append(";containerID=").append(registration.getContainerID()); //$NON-NLS-1$
				buf
						.append(";clazzes=").append(Arrays.asList(registration.getClasses())); //$NON-NLS-1$
				buf
						.append(";reference=").append(registration.getReference()).append("]"); //$NON-NLS-1$ //$NON-NLS-2$
				return buf.toString();
			}
		};
	}
	...
	protected IRemoteServiceRegisteredEvent createRegisteredEvent(
		...
		return new IRemoteServiceRegisteredEvent() {
			...
			public String toString() {
				final StringBuffer buf = new StringBuffer(
						"RemoteServiceRegisteredEvent["); //$NON-NLS-1$
				buf.append("localContainerID=").append(getLocalContainerID()); //$NON-NLS-1$
				buf
						.append(";containerID=").append(registration.getContainerID()); //$NON-NLS-1$
				buf
	 ...
	 * @since 3.2
	 */
	protected RemoteServiceRegistrationImpl getLocalRegistrationForRequest(
			Request request) {
		synchronized (localRegistry) {
	...
	protected boolean handleSharedObjectMsg(SharedObjectMsg msg) {
		try {
			...
			logException(MSG_INVOKE_ERROR_CODE, NLS.bind(
					MSG_INVOKE_ERROR_MESSAGE, msg), e);
		}
		return false;
	}
	 ...
	 * @since 3.0
	 */
	public Future asyncGetRemoteServiceReferences(final ID[] idFilter,
			final String clazz, final RegularExpression filter) {
	 ...
	 * @since 3.0
	 */
	public Future<?> asyncGetRemoteServiceReferences(final ID target,
			final String clazz, final RegularExpression filter) {
	...
	public Namespace getRemoteServiceNamespace() {
		return IDFactory.getDefault().getNamespaceByName(
				RemoteServiceNamespace.NAME);
	}

	/**
	 * @since 3.0
	 */
	public IRemoteFilter createRemoteFilter(RegularExpression filter)
	...
	public IRemoteServiceReference getRemoteServiceReference(
		...
		if (this.localRegistry.containerID.equals(containerID)) {
			synchronized (localRegistry) {
				registration = localRegistry
						.findRegistrationForServiceId(serviceId
	...
	public IRemoteServiceID getRemoteServiceID(ID containerId,
		...
		synchronized (localRegistry) {
			if (localRegistry.containerID.equals(containerId)) {
				RemoteServiceRegistrationImpl reg = localRegistry
						.findRegistrationForServiceId(containerRelativeId);
				if (reg != null)
					return reg.getID();
			}
		}
		synchronized (remoteRegistrys) {
			...
			for (final Iterator i = registrys.iterator(); i.hasNext();) {
				final RemoteServiceRegistryImpl registry = (RemoteServiceRegistryImpl) i
						.next();
				RemoteServiceRegistrationImpl reg = registry
						.findRegistrationForServiceId(containerRelativeId);
				if (reg != null)
					return reg.getID();
			}
